...
namespace XmlNs
{
    public class ImplementateToXml
    {
        ...
        public static string[] ImplementLinear(string intermidateFile, int _MaxDegreeOfParallelism = -1, bool deletedInputFile = false)
        {
            ...
            new MsSql().ExecuteToDB(new string[7] { NameArray, EnvelopeID, DocumentID, PrDocumentName, PrDocumentNumber, DocCode, NewDocToArchName });
        ...
        private void DataImplementationTemplate(string FileName, bool deletedInputFile = false)
        {
            ...
            string NameArray = Path.GetFileName(FileName).Split('.')[0]; // Можно упростить
            var file_xml = new XmlDocument();
            var doc_to_arch = new XmlDocument();

            string PrDocumentName = "", PrDocumentNumber = "", PrDocumentDate = "", DocCode = "", DocName = "";

            string NewDocToArchName = Path.Combine(FilesIntermidate, Path.GetFileName(FileName));
            File.Copy(FileTemplate, NewDocToArchName, true);
